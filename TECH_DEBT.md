...
## Step #15 -- Scenarios financiers : registre canonique et versionne (2026-05-16)

### Contexte

Avant cette etape, les resultats de calcul PV n'existaient que dans
`study_versions.data_json.scenarios_v2` (blob JSONB non indexable). Impossible d'interroger
les ROI, CAPEX ou IRR en SQL, de detecter un recalcul inutile, ou de prouver l'immutabilite
des resultats apres verrouillage.

### Architecture retenue (dual-write additive)

- `study_versions.data_json.scenarios_v2` reste la **source de lecture** — aucun composant
  frontend ni controller ne change sa logique de lecture.
- La table `financial_scenarios` est la **source d'audit et d'analytics** — ecritures
  non-bloquantes (`void .catch()`), jamais en chemin critique du calcul.

### Reference : CP-FINSCEN-001

**Migration** : `backend/migrations/1780500000000_cp-financial-scenarios.js`

Table `financial_scenarios` :
- Cle naturelle : `UNIQUE (study_version_id, scenario_id)`.
- `input_params` : snapshot figé du `solarnextPayload` (JSON).
- `results` : snapshot figé du scenario_v2[i] (JSON).
- `input_hash` / `result_hash` : SHA-256 → deduplication + preuve integrite.
- `engine_version` : constante `ENGINE_VERSION` depuis `backend/constants/engineVersion.js`.
- Colonnes denormalisees : `capex_ttc`, `roi_years`, `irr_pct` pour SQL analytique.
- Cycle de vie : `DRAFT` → `LOCKED` (scénario selectionné) / `ARCHIVED` (autres scénarios).

### Fichiers crees

- **`backend/constants/engineVersion.js`** :
  - Exporte `ENGINE_VERSION = "calc-v2026-05"`.
  - A incrementer a chaque modification du moteur produisant des resultats differents.

- **`backend/services/financialScenarios.service.js`** :
  - `upsertFinancialScenariosForVersion({ organizationId, studyId, studyVersionId, solarnextPayload, scenariosV2, userId })` :
    - Upserte un enregistrement par scenario present dans `scenariosV2`.
    - ON CONFLICT (study_version_id, scenario_id) DO UPDATE → si LOCKED avant, repasse a DRAFT.
    - Hash deterministique via `JSON.stringify(obj, Object.keys(obj).sort())`.
  - `lockFinancialScenario(studyVersionId, scenarioId)` :
    - Scénario selectionne → `LOCKED` + `locked_at = NOW()`.
    - Autres scénarios DRAFT de la meme version → `ARCHIVED`.

### Fichiers modifies

- **`backend/controllers/studyCalc.controller.js`** :
  - Import `upsertFinancialScenariosForVersion`.
  - Appel non-bloquant apres `UPDATE study_versions` (ligne ~238).
  - Jamais en chemin de retour HTTP — erreur logguee, pas remontee.

- **`backend/controllers/selectScenario.controller.js`** :
  - Import `lockFinancialScenario`.
  - Appel non-bloquant apres `UPDATE study_versions SET is_locked = true`.

### Garanties

- **Zero impact sur le flux metier** : tous les appels sont `void .catch(console.error)`.
- **Backward compat** : les enregistrements anterieurs a la migration sont absents de la table
  (pas de backfill possible — inputs non isoles dans les anciens JSONB).
- **Idempotence** : ON CONFLICT DO UPDATE → un recalcul sur la meme version ecrase l'entree.
