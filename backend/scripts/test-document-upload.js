/**
 * CP-032 — Tests Upload Documents (Infomaniak S3)
 * Usage: cd backend && node scripts/test-document-upload.js
 *
 * Teste :
 * - Création client
 * - Upload fichier test
 * - Vérification en DB
 * - Vérification S3 object (si configuré)
 * - Suppression
 * - Vérification suppression S3 + DB
 */
...
async function ensureTestClient(pool) {
  ...
  try {
    ...
    const ins = await client.query(
      ...
      [orgId, clientNum, "Test Document Client"]
    );
    return { clientId: ins.rows[0].id };
  } finally {
    client.release();
  }
}
...
async function main() {
  ...
  try {
    ...
    formData.append("file", Buffer.from("Contenu fichier test CP-032"), {
      filename: "test-cp032.txt",
    ...
    if (uploadRes.status === 503) {
      ...
      fail("Upload", errData.error || "S3 non configuré ou non accessible");
    ...
    const listRes = await fetch(`${BASE_URL}/api/documents/client/${testData.clientId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (listRes.status !== 200) {
      fail("GET documents", `status ${listRes.status}`);
      throw new Error("Liste documents échouée");
    }
    const list = await listRes.json();
    if (!Array.isArray(list) || list.length === 0) {
      fail("GET documents", "Liste vide");
      throw new Error("Liste vide");
    }
    ok("GET documents retourne la liste");
