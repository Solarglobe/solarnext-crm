...
(function (global) {
  ...
  /**
   * Recalcule halfLengthAlongSlopePx / halfLengthPerpPx à partir des sommets et des axes (après rotation bloc ou locale).
   * @param {{ x: number, y: number }} center - Centre de référence (souvent centre de pose / rotation).
   * @param {Array<{ x: number, y: number }>} points - Polygone projeté (ex. 4 sommets).
   * @param {{ x: number, y: number }} slopeAxis - Axe pente (non nécessairement unitaire).
   * @param {{ x: number, y: number }} perpAxis - Axe perpendiculaire à la pente.
   * @returns {{ halfLengthAlongSlopePx: number, halfLengthPerpPx: number }}
   */
  function recomputeProjectionHalfExtentsFromGeometry(center, points, slopeAxis, perpAxis) {
    if (!center || typeof center.x !== "number" || typeof center.y !== "number" || !points || points.length === 0) {
      return { halfLengthAlongSlopePx: 0, halfLengthPerpPx: 0 };
    }
    ...
    for (var i = 0; i < points.length; i++) {
      var pt = points[i];
      var px = pt.x != null ? pt.x : pt[0];
      var py = pt.y != null ? pt.y : pt[1];
      var s = (px - cx) * ux + (py - cy) * uy;
      var p = (px - cx) * vx + (py - cy) * vy;
      if (s < minS) minS = s;
      if (s > maxS) maxS = s;
      if (p < minP) minP = p;
      if (p > maxP) maxP = p;
    ...
    return {
      halfLengthAlongSlopePx: (maxS - minS) / 2,
      halfLengthPerpPx: (maxP - minP) / 2,
    };
  }
  ...
  var PanelProjection = {
    ...
    recomputeProjectionHalfExtentsFromGeometry: recomputeProjectionHalfExtentsFromGeometry,
  };
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    global.recomputeProjectionHalfExtentsFromGeometry = recomputeProjectionHalfExtentsFromGeometry;
  }
