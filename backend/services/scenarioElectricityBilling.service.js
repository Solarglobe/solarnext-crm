import { getCalendar, bindCalendar, isEnergyYear } from './energyCalendar.service.js';
...
const series = (v) => isEnergyYear(v) ? v : null;
...
export function priceElectricitySeries(values, contract, fallbackKwh = null) {
  ...
  const fractions = buildHpHcHourlyFractions(contract.off_peak_periods, getCalendar(hourly));
...
export function resolveVirtualSupplyPricing(ctx, vbSim, contractType = null, meterKva = null) {
  ...
  const contract = resolveVirtualElectricityContract({
    ...
    tariffReferenceDate: input.tariff_reference_date ?? null,
  ...
  let energyCost = priceElectricitySeries(hourly, contract, kwh);
  if (vbSim?.commercial_ledger && contract.contract_type === 'HPHC') {
    energyCost = vbSim.commercial_ledger.billable_hp_kwh * contract.price_hp_eur_kwh + vbSim.commercial_ledger.billable_hc_kwh * contract.price_hc_eur_kwh;
  }
...
export function attachScenarioElectricityBilling(scenarios, ctx, physicalBattery = null, {currentContract=null}={}) {
  const current = currentContract ?? resolveCurrentElectricityContract(ctx);
  ...
  const calendar=getCalendar(load,ctx.conso?.calendar);
  if(load)bindCalendar(load,calendar);
  if(pv)bindCalendar(pv,calendar);
  const currentEnergy = priceElectricitySeries(load, current, ctx.conso?.total_kwh);
  const direct = load && pv ? bindCalendar(load.map((v, h) => Math.min(v, pv[h])),calendar) : null;
  ...
  for (const [key, sc] of Object.entries(scenarios)) {
    ...
    for (const values of [autoHourly,importHourly]) if(values)bindCalendar(values,calendar);
    ...
    if(creditHourly)bindCalendar(creditHourly,calendar);
    ...
    const variableCost=virtual ? finite(vf?.annual_virtual_discharge_cost_ttc) ?? 0 : 0;
    const usedCredit=creditHourly?sum(creditHourly):0;
    const restitution={rate_base_ttc:finite(vf?.virtual_discharge_rate_base_ttc_per_kwh),rate_hp_ttc:finite(vf?.virtual_discharge_rate_hp_ttc_per_kwh),rate_hc_ttc:finite(vf?.virtual_discharge_rate_hc_ttc_per_kwh),effective_rate_ttc:usedCredit>0?variableCost/usedCredit:0,
      used_hp_kwh:vb?.commercial_ledger?.used_hp_kwh??vf?.discharged_hp_kwh??null,used_hc_kwh:vb?.commercial_ledger?.used_hc_kwh??vf?.discharged_hc_kwh??null,provider_rules_id:vb?.provider_rules?.id??null,
      tariff_edition_id:vf?.tariff_edition_id??null,tariff_reference_date:vf?.tariff_reference_date??null,tariff_effective_date:vf?.tariff_effective_date??null,
      tax_treatment:vf?.restitution_tax_treatment??null,source_urls:vf?.tariff_source_urls??null};
    const displayedBefore=complete?money(beforeBill):null,displayedAfter=complete?money(afterBill):null;
    const billing = {
      ...
      virtual_service_fixed_cost_eur: service==null?null:Math.max(0,service-variableCost),
      virtual_restitution_cost_eur: variableCost,
      virtual_restitution_price_eur_kwh: restitution.effective_rate_ttc,
      virtual_restitution:restitution,
      included_autoproducer_contribution_eur: money(includedContribution),
      legacy_initial_service_fee_eur: legacyInitialFee,
      bill_before_eur: displayedBefore,
      bill_after_eur: displayedAfter,
      bill_savings_eur: complete ? money(displayedBefore - displayedAfter) : null,
