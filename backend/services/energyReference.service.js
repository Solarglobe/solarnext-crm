import {isEnergyYear,getCalendar,calendarParts,monthlySums} from './energyCalendar.service.js';
import { createHash } from "node:crypto";

export const ENERGY_REFERENCE_VERSION = "ac-energy-ledger-2";
const DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
const sum = (xs) => xs.reduce((a, b) => a + b, 0);
const ratio = (a, b) => b > 0 ? a / b : null;

export function assertEnergyProfile(values, name, length = null) {
  if (!isEnergyYear(values) || (length != null && values.length !== length)) throw new Error(`${name}: ${length} intervalles horaires requis`);
  values.forEach((v, i) => {
...
export function buildEnergyReference({ pv, load, battery = null, scenarioId = "BASE", provenance = null, injectionLimitKw = null }) {
  ...
  assertEnergyProfile(load, "consommation",pv.length);
  ...
  if (battery) {
    for (const key of ["direct_self_consumption_hourly", "batt_charge_input_hourly", "batt_discharge_hourly", "surplus_hourly", "battery_soc_hourly", "battery_losses_hourly"]) assertEnergyProfile(battery[key], key,pv.length);
  ...
  const calendar=getCalendar(load);
  const sums=Object.fromEntries(keys.map(k=>[k,monthlySums(hourly[k],calendar)]));
  const monthly=Array.from({length:12},(_,m)=>Object.fromEntries(keys.map(k=>[k,sums[k][m]])));
  ...
  return {
    ...
    input_hash: createHash("sha256").update(JSON.stringify({ pv, load, calendar, battery: battery?.resolved_parameters ?? null, injectionLimitKw, scenarioId })).digest("hex"),
    ...
    annual,
    monthly,
    daily_average: Object.fromEntries(Object.entries(hourly).map(([k,series])=>[k,Array.from({length:24},(_,h)=>series.reduce((s,v,i)=>calendarParts(calendar)[i].hour===h?s+v:s,0)/(pv.length/24))])),
    ratios: {
