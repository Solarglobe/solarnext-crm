...
export function validateStudyScenarioForExport(scenario, expectedScenarioId) {
  ...
  if(!ref || ref.version!==ENERGY_REFERENCE_VERSION)errors.push("ENERGY_REFERENCE_MISSING: recalculer l'etude avec le moteur courant");
    ...
    if(expectedScenarioId.includes("VIRTUAL")||expectedScenarioId==="BATTERY_HYBRID") {
      ...
      else if(Math.abs(credit.opening_kwh+credit.credited_kwh-credit.used_kwh-credit.closing_kwh)>energyTolerance(credit.credited_kwh))errors.push("VIRTUAL_CREDIT_BALANCE_INVALID");
      if(credit?.monthly?.length===12){
  ...
  if(!f || !Number.isFinite(f.capex_ttc) || !Array.isArray(f.annual_cashflows)||!f.annual_cashflows.length)errors.push("FINANCIAL_CASHFLOWS_MISSING");
    ...
    f.annual_cashflows.forEach((row,i)=>{
      ...
      if(row.gain_auto!=null){
        ...
        const parts=[row.gain_auto,row.gain_oa,row.prime,row.maintenance,row.inverter_cost,row.battery_cost??0,row.virtual_service_cost_eur??0,row.import_savings_eur??0,
        ...
        const total=hasBillLedger
          ? billSavings+row.gain_oa+row.prime-row.maintenance-row.inverter_cost-(row.battery_cost??0)-(row.initial_virtual_service_fees_eur??0)
          : row.gain_auto+row.gain_oa+row.prime+(row.import_savings_eur??0)-row.maintenance-row.inverter_cost-(row.battery_cost??0)-(row.virtual_service_cost_eur??0);
    ...
    warnings.push("Projection annuelle de vieillissement estimee, sans recalcul horaire futur.");
  }
