...
export function applyVerifiedEnergyPresentation(vm, ref, scenario = {}) {
  if (!ref || ref.version !== "ac-energy-ledger-1") return vm;
  const a = ref.annual;
  const q = Math.round;
  ...
  if(ref.daily_average) Object.assign(fr.p5, {
    ...
    profile_notes:{production:"Moyenne annuelle des 365 jours de simulation, par heure UTC ; journée illustrative.",consumption:"Même série horaire que les bilans ; les moyennes des flux directs sont calculées avant agrégation."},
  });
  ...
  if (credit) {
    ...
    if (Array.isArray(hourly) && hourly.length === 8760 && hourly.every(v => Number.isFinite(v) && v >= 0)) {
      if (Math.abs(hourly.reduce((s,v)=>s+v,0)-credit.used_kwh)>0.01) throw new Error("PDF_VIRTUAL_CREDIT_HOURLY_MISMATCH");
      fr.p5.credit_kw = Array.from({length:24},(_,h)=>hourly.reduce((s,v,i)=>i%24===h?s+v:s,0)/365);
    }
