import {isEnergyYear,copyEnergy} from '../energyCalendar.service.js';
// ======================================================================
...
export function buildScenarioBaseV2(ctx) {
  const pvHourly = isEnergyYear(ctx?.pv?.hourly)
    ? copyEnergy(ctx.pv.hourly)
    : null;
  const consoHourly = isEnergyYear(ctx?.conso?.hourly)
      ? copyEnergy(ctx.conso.hourly)
      : isEnergyYear(ctx?.conso?.clamped)
        ? copyEnergy(ctx.conso.clamped)
        : null;
