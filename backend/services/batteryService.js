import {isEnergyYear,getCalendar,bindCalendar} from './energyCalendar.service.js';
// ======================================================================
...
function passThroughNoBattery(pv_hourly, conso_hourly) {
  ...
  return {
    ...
    surplus_hourly,
    batt_discharge_hourly: Array(pv_hourly.length).fill(0),
    batt_charge_input_hourly: Array(pv_hourly.length).fill(0),
    batt_charge_hourly: Array(pv_hourly.length).fill(0),
    battery_soc_hourly: Array(pv_hourly.length).fill(0),
    prod_kwh: pv_total,
    auto_kwh: auto_total,
...
export function simulateBattery8760({
  ...
  const socEpsilon=32*Number.EPSILON*Math.max(1,capacity_kwh);
  if (![SOC_min, maxSOC, SOC].every(Number.isFinite) || SOC_min < -socEpsilon || maxSOC > capacity_kwh+socEpsilon || maxSOC < SOC_min-socEpsilon || SOC < SOC_min-socEpsilon || SOC > maxSOC+socEpsilon) return { ok: false, reason: "INVALID_BATTERY_SOC" };
  SOC=Math.max(SOC_min,Math.min(maxSOC,SOC));
  ...
  const calendar=getCalendar(conso_hourly);
  for (const values of [auto_hourly,direct_self_consumption_hourly,surplus_before_battery_hourly,surplus_hourly,batt_discharge_hourly,batt_charge_input_hourly,batt_charge_hourly,battery_soc_hourly,battery_losses_hourly]) bindCalendar(values,calendar);
  const pv_total = pv_hourly.reduce((a, b) => a + (b || 0), 0);
