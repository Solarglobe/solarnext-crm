...
export function createTreeShadingRouter({loadStudy,dataDir,acquire=acquireScene,calculate=runCalculation,renderPdf=renderTreePdf}){
  ...
  router.use(async(req,res,next)=>{try{
    ...
    const geometryHash=hash(study.geometry);req.tree={study,directory,state,geometryHash,org,version,stale:!!state&&state.geometryHash!==geometryHash};next();
  }catch(e){next(e);}});
  ...
  async function recheck(req){const fresh=await loadStudy(req.tree.org,req.params.studyId,req.tree.version);if(!fresh||hash(fresh.geometry)!==req.tree.geometryHash)throw error(409,'Calepinage modifié pendant le traitement');}
  ...
  router.post('/acquire',job(async(req,res)=>{
    ...
    const scene=await acquire(clone,ctx.directory);scene.title=ctx.study.title||'Analyse arbres';scene.context='crm';scene.id=ctx.study.id;await recheck(req);
    const state={scene,result:null,geometryHash:ctx.geometryHash,revision:0};await persist(ctx,state);res.json(state);
  }));
  router.post('/scene',job(async(req,res)=>{
    ...
    let scene={...state.scene,trees:edited,emptySceneAttested:req.body.emptySceneAttested===true};
  ...
  router.post('/calculate',job(async(req,res)=>{const ctx=req.tree,state=current(ctx),irradiation=JSON.parse(await fs.readFile(path.join(ctx.directory,'irradiation.json'),'utf8'));const result=await calculate(state.scene,irradiation);await recheck(req);await persist(ctx,{...state,result});res.json(result);}));
