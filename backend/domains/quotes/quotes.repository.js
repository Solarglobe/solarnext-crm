...
import {
  getRecentVerifiedOtp,
  resolveQuoteSignerContact,
  orgHasActiveMailbox,
} from "../../services/quoteSignatureOtp.service.js";
import { isSmsConfigured } from "../../services/smsPartner.service.js";
...
export async function finalizeQuoteSigned(quoteId, organizationId, userId, body = {}, context = {}) {
  ...
  const signerInfo = await resolveQuoteSignerContact(quoteId, organizationId);
  let otpProof = null;
  const verificationPossible = Boolean(signerInfo.email || signerInfo.phone);
  if (verificationPossible) {
    otpProof = await getRecentVerifiedOtp(quoteId, organizationId);
    const deliverable =
      isSmsConfigured() || isSystemMailConfigured() || (await orgHasActiveMailbox(organizationId));
    if (!otpProof && deliverable) {
      const err = new Error(
        "Vérification d'identité requise : envoyez le code de signature au client (email ou SMS) et validez-le avant de finaliser."
      );
      err.statusCode = 400;
      throw err;
    }
  ...
  const signatureEvidence = {
    ...
    otp: otpProof
      ? {
          channel: otpProof.channel,
          destination: otpProof.destination,
          email: otpProof.email,
          verifiedAt: otpProof.verifiedAt,
        }
      : signerInfo.email || signerInfo.phone
        ? { channel: null, verifiedAt: null, skipped: "not_verified_or_mail_unavailable" }
        : { channel: null, skipped: "no_signer_contact" },
    cgv: cgvRaw
      ? {
  ...
  try {
    ...
    pdfBuffer = await appendSignatureCertificatePage(pdfBuffer, {
      ...
      otp: signatureEvidence.otp?.verifiedAt
        ? {
            channel: signatureEvidence.otp.channel ?? "email",
            destination: signatureEvidence.otp.destination ?? signatureEvidence.otp.email ?? null,
            verifiedAtLabel: formatFrDateTime(signatureEvidence.otp.verifiedAt),
          }
        : null,
      signaturePlace,
