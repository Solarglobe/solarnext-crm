/**
 * Moteur d'ombrage.
 * P3/7 : extrusion obstacles + base géométrie raycast (sans impact moteur PV).
 * P4/7 : sampling annuel + boucle solaire (safe, sans impact).
 * P5/7 : raycast panneau + fraction ombrée, annualLossPercent calculé.
 * Pas d'exposition globale.
 */

const { computeSunPosition } = require("./solarPosition");
...
function normalizeObstacles(obstacles) {
  ...
  for (var i = 0; i < obstacles.length; i++) {
    ...
    if (shape === "circle") {
      ...
      var ptsFallback = o.points || o.polygon;
...
function samplePanelPoints(panel) {
  if (!panel) return [];
  var polygon = panel.polygon || panel.points || (panel.projection && panel.projection.points);
...
/**
 * Génère les échantillons annuels (dates) où le soleil est au-dessus de l'horizon.
...
function generateAnnualSamples(opts, latDeg, lonDeg) {
  ...
  var samples = [];
  var startMs = new Date(year, 0, 1, 0, 0, 0).getTime();
  var endMs = new Date(year, 11, 31, 23, 59, 0).getTime();
  ...
  return { samples: samples, totalCount: samples.length };
}

/**
 * API principale : calcule la perte annuelle d'ombrage.
 * P5/7 : raycast panneau + fraction ombrée, annualLossPercent calculé.
...
function computeAnnualShadingLoss(params) {
  ...
  for (var s = 0; s < samples.length; s++) {
    ...
    // OMBRAGE LOINTAIN — masque d'horizon
    if (typeof window !== "undefined" &&
        window.isSunBlockedByHorizonMask &&
        window.isSunBlockedByHorizonMask(azDeg, elDeg)) {
      continue; // Soleil masqué → aucune contribution
    }
  ...
  var annualLossPercent =
    (totalWeight <= 0)
      ? 0
      : clamp01(totalWeightedFraction / totalWeight) * 100;
