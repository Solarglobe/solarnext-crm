...
function displayResult(){
  ...
  if(result?.scope==='combined-shading-v1'){document.querySelector('.results h2').textContent='Analyse d’ombrage — perte globale';document.querySelector('.heading h1').textContent='Analyse d’ombrage';}
  ...
  if(result.components){const p=document.createElement('p');p.textContent=`Composantes énergétiques sans double comptage : arbres ${pct(result.components.trees.lossPercent)}, obstacles proches ${pct(result.components.obstacles.lossPercent)}, relief et horizon ${pct(result.components.horizon.lossPercent)}.`;out.append(p);}
