...
(function (global) {
  ...
  function renderPanPropertiesPanel(options) {
    ...
    function syncAzimuthFromPan(pan) {
      var v = (pan.physical && pan.physical.orientation && pan.physical.orientation.azimuthDeg != null)
        ? pan.physical.orientation.azimuthDeg : (pan.azimuthDeg != null ? pan.azimuthDeg : 0);
      v = clampAzimuth(v);
      if (azimuthSlider) azimuthSlider.value = String(v);
      if (azimuthInput) azimuthInput.value = String(v);
    }
    function syncTiltFromPan(pan) {
      var v = (pan.physical && pan.physical.slope && pan.physical.slope.valueDeg != null)
        ? pan.physical.slope.valueDeg : (pan.tiltDeg != null ? pan.tiltDeg : TILT_DEFAULT_HINT);
      v = clampTilt(v);
      if (tiltSlider) tiltSlider.value = String(v);
    ...
    function buildControls() {
      ...
      azimuthInput.addEventListener("input", function () {
        var pan = getActivePan();
        if (!pan) return;
        var v = clampAzimuth(Number(azimuthInput.value));
        pan.azimuthDeg = v;
        if (azimuthSlider) azimuthSlider.value = String(v);
        onRedraw();
      });
      ...
      tiltInput.addEventListener("input", function () {
        var pan = getActivePan();
        if (!pan) return;
        var v = clampTilt(Number(tiltInput.value));
        if (typeof onApplyManualSlope === "function") {
          onApplyManualSlope(pan, v);
        } else {
          pan.tiltDeg = v;
  ...
  /** Assure que chaque pan a azimuthDeg, tiltDeg, physical (structure) et que chaque point a h (0 si absent). */
  ...
  /** Azimut 0°=Nord, 90°=Est... à partir d'un vecteur image (dx, dy) où dy positif = bas. */
  function azimuthFromImageVector(dx, dy) {
    var ang = Math.atan2(dx, -dy) * DEG_PER_RAD;
    ang = ((ang % 360) + 360) % 360;
    return ang;
  }

  /** Corrige l'azimut image avec l'angle Nord du toit (soustraction). Conservé pour compatibilité, préférer applyNorth. */
  function applyNorthCorrection(azImageDeg, state) {
    var north = 0;
    if (state && state.roof) {
      if (state.roof.north && typeof state.roof.north.angleDeg === "number") north = state.roof.north.angleDeg;
      else if (state.roof.roof && state.roof.roof.north && typeof state.roof.roof.north.angleDeg === "number") north = state.roof.roof.north.angleDeg;
    }
    var az = azImageDeg - north;
    az = ((az % 360) + 360) % 360;
    return az;
  }
  ...
  /** Azimut vrai = azimut image + angle Nord (convention : on ADD). */
  function applyNorth(azImageDeg, state) {
    var northAngleDeg = getNorthAngleDeg(state);
    return normalizeDeg(azImageDeg + northAngleDeg);
  }

  /** EPS_H = 0.10 m pour regrouper points bas/haut (gouttière / faîtage). */
  var EPS_H_M = 0.10;

  /** Sépare les points en ensembles bas (gouttière) et haut (faîtage) via hauteurs; fallback min/max exact si vide. */
  function pickLowHighSets(pts, state) {
    if (!pts || pts.length === 0) return { minH: 0, maxH: 0, lowPts: [], highPts: [] };
    var minH = getVertexH(pts[0], state), maxH = minH;
    for (var i = 1; i < pts.length; i++) {
      var h = getVertexH(pts[i], state);
      if (h < minH) minH = h;
      if (h > maxH) maxH = h;
    }
    var lowPts = pts.filter(function (p) { return getVertexH(p, state) <= minH + EPS_H_M; });
    var highPts = pts.filter(function (p) { return getVertexH(p, state) >= maxH - EPS_H_M; });
    if (lowPts.length === 0) {
      lowPts = pts.filter(function (p) { return getVertexH(p, state) === minH; });
      if (lowPts.length === 0) lowPts = [pts[0]];
    }
    if (highPts.length === 0) {
      highPts = pts.filter(function (p) { return getVertexH(p, state) === maxH; });
      if (highPts.length === 0) highPts = [pts[0]];
    }
    return { minH: minH, maxH: maxH, lowPts: lowPts, highPts: highPts };
  }

  function horizontalDistanceM(a, b, metersPerPixel) {
    var dx = b.x - a.x, dy = b.y - a.y;
    return Math.hypot(dx, dy) * metersPerPixel;
  }

  function fitPlane(pts, metersPerPixel, state) {
    ...
    for (var i = 0; i < n; i++) {
      var xM = pts[i].x * metersPerPixel, yM = pts[i].y * metersPerPixel, h = getVertexH(pts[i], state);
  ...
  /** Pente auto par centroïdes (gouttière / faîtage), pas par paire min/max. runM < 0.20 m → 0° (évite 88–89°). */
  function computePanSlopeComputedDeg(pan, state) {
    ...
    var runPx = distImage(lowC, highC);
    var runM = runPx * mpp;
    if (runM < 0.20) return { deg: 0, runM: runM, deltaH: deltaH, lowC: lowC, highC: highC };
    var deg = Math.atan2(deltaH, runM) * DEG_PER_RAD;
    return { deg: deg, runM: runM, deltaH: deltaH, lowC: lowC, highC: highC };
  }
  ...
  function getDescentVector(pan, state) {
    ...
    if (!pts || pts.length < 2) return null;
    var mpp = (state.roof && state.roof.scale && state.roof.scale.metersPerPixel) != null ? state.roof.scale.metersPerPixel : 1;
    var plane = fitPlane(pts, mpp, state || null);
  ...
  /** Orientation = sens descendant faîtage → gouttière. lowC = gouttière, highC = faîtage. Azimut 0°=Nord, 90°=Est, 180°=Sud, 270°=Ouest. applyNorth = ADD. */
  function computePanOrientation(pan, state) {
    ...
    var lowC = avgPoint(sets.lowPts);
    var highC = avgPoint(sets.highPts);
    var dx = lowC.x - highC.x, dy = lowC.y - highC.y;
    if (Math.abs(dx) < TOL && Math.abs(dy) < TOL) return null;
    var azImage = azimuthFromImageVector(dx, dy);
    var azTrue = applyNorth(azImage, state);
  ...
  function recomputePanPhysicalProps(pan, state) {
    ...
    if (computedDeg != null) pan.physical.slope.computedDeg = computedDeg;
    if (slopeResult) {
      if (slopeResult.runM != null) pan.physical.slope._runM = slopeResult.runM;
      if (slopeResult.deltaH != null) pan.physical.slope._deltaH = slopeResult.deltaH;
    }
    if (pan.physical.slope.mode === "auto" && computedDeg != null) {
      pan.physical.slope.valueDeg = isFiniteNum(computedDeg) ? computedDeg : 0;
      if (!isFiniteNum(computedDeg) && typeof window !== "undefined" && window.__CALPINAGE_HEIGHT_DIAG__) {
        console.warn("[recomputePanPhysicalProps] computedDeg non finite pan=" + (pan.id || "?") + " -> fallback 0");
      }
    }
    var orient = computePanOrientation(pan, state);
    if (orient) {
      var az = orient.azimuthDeg;
      pan.physical.orientation.azimuthDeg = isFiniteNum(az) ? az : 0;
      if (!isFiniteNum(az) && typeof window !== "undefined" && window.__CALPINAGE_HEIGHT_DIAG__) {
        console.warn("[recomputePanPhysicalProps] azimuthDeg non finite pan=" + (pan.id || "?") + " -> fallback 0");
      }
    ...
    var vAz = pan.physical.orientation.azimuthDeg;
    var vTilt = pan.physical.slope.valueDeg;
    if (vAz != null) pan.azimuthDeg = isFiniteNum(vAz) ? vAz : 0;
    if (vTilt != null) pan.tiltDeg = isFiniteNum(vTilt) ? vTilt : 0;
  }
  ...
  function getPanPoints(pan) {
    ...
    if (pan.polygon && pan.polygon.length >= 2) {
      return pan.polygon.map(function (p, i) { return { x: p.x, y: p.y, h: 0, id: pan.id + "-" + i }; });
    }
    return [];
  }
  ...
  function applyManualSlopeToPan(pan, desiredSlopeDeg, state) {
    ...
    for (var i = 0; i < workPts.length; i++) {
      ...
      for (var j = 0; j < workPts.length; j++) {
        ...
        var run = horizontalDistanceM(workPts[i], workPts[j], mpp);
  ...
  /**
   * API officielle : calcule la hauteur réelle Z (m) d'un point (X,Y) situé sur un pan.
   * Utilise le plan ajusté h = a*xM + b*yM + c (fitPlane).
   * @param {string} panId - Identifiant du pan
   * @param {number} xPx - Coordonnée X en pixels image
   * @param {number} yPx - Coordonnée Y en pixels image
   * @param {{ pans: Array, roof?: { scale?: { metersPerPixel: number } }, getVertexH?: function }} state - State (getStateForPans())
   * @returns {number|null} - Hauteur en mètres, ou null si plan invalide
   */
  function getHeightAtXY(panId, xPx, yPx, state) {
    ...
    console.log("[getHeightAtXY] panId=" + panId + ", xM=" + xM + ", yM=" + yM + ", h=" + h);
    return h;
  }
