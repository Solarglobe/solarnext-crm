 ...
 * RDV-LEAD : les leads non convertis voient et créent aussi leurs rendez-vous.
 */
...
interface RdvTabProps {
  ...
  onNewRdv: () => void;
  /** CP-LEAD-CLIENT-UNIFICATION — Si true (Lead), texte adapté */
  isLead?: boolean;
}
    ...
    <section className="crm-lead-card">
      ...
      <div className="crm-lead-fields" style={{ marginBottom: 16 }}>
        <button type="button" className="sn-btn sn-btn-primary" onClick={onNewRdv}>
          {isLead ? "Planifier un rendez-vous" : "Nouveau rendez-vous"}
        </button>
      </div>
      {missionsLoading ? (
