...
describe("buildDormerMesh", () => {
  ...
  it("redresse un faitage canonique manuel reste a h=0 par defaut", () => {
    const patch = {
      id: "pan-test",
      cornersWorld: [
        { x: 0, y: 0, z: 5 },
    ...
    const ext = {
      ...
      canonicalDormerGeometry: {
        vertices: [
          { id: "b0", role: "base_contour", x: 10, y: 10, h: 0 },
          { id: "b1", role: "base_contour", x: 20, y: 10, h: 0 },
          { id: "b2", role: "base_contour", x: 20, y: 20, h: 0 },
          { id: "b3", role: "base_contour", x: 10, y: 20, h: 0 },
          { id: "e0", role: "wall_eave", x: 10, y: 10, h: 0.35 },
          { id: "e1", role: "wall_eave", x: 20, y: 10, h: 0.35 },
          { id: "e2", role: "wall_eave", x: 20, y: 20, h: 0.35 },
          { id: "e3", role: "wall_eave", x: 10, y: 20, h: 0.35 },
          { id: "r0", role: "ridge", x: 12, y: 15, h: 0 },
          { id: "r1", role: "ridge", x: 18, y: 15, h: 0 },
        ],
        faces: [
          { id: "wall-front", vertexIds: ["b0", "b1", "e1", "e0"] },
          { id: "wall-right", vertexIds: ["b1", "b2", "e2", "e1"] },
          { id: "wall-back", vertexIds: ["b2", "b3", "e3", "e2"] },
          { id: "wall-left", vertexIds: ["b3", "b0", "e0", "e3"] },
          { id: "roof-front-slope", vertexIds: ["e0", "e1", "r1", "r0"] },
          { id: "roof-back-slope", vertexIds: ["e3", "r0", "r1", "e2"] },
          { id: "gable-left", vertexIds: ["e0", "r0", "e3"] },
          { id: "gable-right", vertexIds: ["e1", "e2", "r1"] },
        ],
      },
    } as const;

    const geo = buildDormerMesh(ext, {
      world: { metersPerPixel: 1, northAngleDeg: 0, referenceFrame: "LOCAL_IMAGE_ENU" },
      roofPlanePatches: [patch],
    });
  ...
  it("rend un chien assis manuel avec contour et faitage meme sans geometrie canonique", () => {
    ...
    let maxZ = -Infinity;
    for (let i = 0; i < pos.count; i++) maxZ = Math.max(maxZ, pos.getZ(i));
    expect(maxZ).toBeGreaterThan(5.5);
  });
});
