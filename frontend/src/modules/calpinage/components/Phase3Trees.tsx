...
export function Phase3Trees({studyId,versionId,prepare,panelCount}:{studyId:string;versionId:string;prepare?:()=>Promise<boolean>;panelCount:number}){
 ...
 async function skip(){setBusy(true);try{await shadingRequest(studyId,versionId,'/skip',{});await refresh();window.dispatchEvent(new CustomEvent('shading:continue',{detail:{key:shadingKey(studyId,versionId)}}));}catch(e){setMessage(String(e));}finally{setBusy(false);}}
 ...
 return <section aria-label="Analyse d’ombrage" style={{padding:14,border:'1px solid #cbd5e1',borderRadius:10,margin:'12px 0'}}>
  ...
  {!panelCount?<p>Placez les panneaux avant de valider le calepinage.</p>:result?<p><strong style={{fontSize:26}}>{pct(result.lossPercent)}</strong><br/>Perte annuelle globale</p>:running?<p role="status">{steps[job.step]||'Analyse en cours'} · {elapsed} s<br/><small>Traitement {job.id.slice(0,8)}</small></p>:excluded?<p>Étude sans analyse d’ombrage.</p>:job?.status==='failed'?<p role="status">Analyse non disponible. {job.error}</p>:<p>Le calcul se lance automatiquement à la validation.</p>}
  ...
  {job?.status==='failed'&&!running&&!excluded&&<button className={styles.toolGhostBtn} disabled={busy||locked} onClick={skip}>Continuer sans analyse d’ombrage</button>}
