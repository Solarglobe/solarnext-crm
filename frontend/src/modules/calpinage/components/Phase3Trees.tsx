...
export function Phase3Trees({studyId,versionId,prepare,panelCount,onContinueWithoutShading}:{studyId:string;versionId:string;prepare?:()=>Promise<boolean>;panelCount:number;onContinueWithoutShading?:()=>Promise<void>}){
 ...
 return <section aria-label="Analyse d’ombrage" style={{padding:14,border:'1px solid #cbd5e1',borderRadius:10,margin:'12px 0'}}>
  ...
  {!panelCount?<p>Placez les panneaux avant de valider le calepinage.</p>:result?<p><strong style={{fontSize:26}}>{pct(result.lossPercent)}</strong><br/>Perte annuelle globale</p>:running?<p role="status">{steps[job.step]||'Analyse en cours'} · {elapsed} s<br/><small>Vous pouvez valider le calepinage sans attendre la fin de l’analyse.</small></p>:excluded?<p>Ombrage non évalué. Aucune perte n’a été calculée.</p>:job?.status==='failed'?<p role="status">{job.error || 'Analyse non disponible.'}<br/><small>Vous pouvez enregistrer le calepinage et continuer sans cette analyse.</small></p>:<p>L’analyse se lance en arrière-plan à la validation. Vous pouvez valider le calepinage sans attendre.</p>}
