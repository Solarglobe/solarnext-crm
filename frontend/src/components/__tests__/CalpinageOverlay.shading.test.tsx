...
it('direct validation saves and starts one durable job without waiting for its result',async()=>{mount();fireEvent.click(screen.getByText('Valider le calepinage'));mocks.validate({geometry_json:{frozenBlocks:[{panels:[{}]}]}});await waitFor(()=>expect(posts).toBe(1));expect(saves).toBe(1);await waitFor(()=>expect(validated).toBe(1));expect(job.status).toBe('running');expect(result).toBeNull();expect(mocks.navigate).toHaveBeenCalledWith('/studies/fictional/quote-builder');expect(sessionStorage.getItem(shadingResumeKey('fictional','1'))).toBeNull();});
it('reload reuses a server job and validates without posting another calculation',async()=>{job={id:'existing-job',status:'running',step:'annual'};sessionStorage.setItem(shadingResumeKey('fictional','1'),'pending');mount();await waitFor(()=>expect(saves).toBe(1));expect(posts).toBe(0);await waitFor(()=>expect(validated).toBe(1));expect(result).toBeNull();expect(posts).toBe(0);});
it('failed acquisition never validates implicitly; explicit continuation saves current panels before excluding shading and validating',async()=>{
 failed=true;job={id:'failed',status:'failed',error:'IGN indisponible'};mount();expect(validated).toBe(0);expect(result).toBeNull();
 ...
 await mocks.continueWithout();expect(order).toEqual(['save','skip','validate']);expect(validated).toBe(1);expect(posts).toBe(0);expect(result).toBeNull();expect(mocks.navigate).toHaveBeenCalledOnce();
});

it.each(['failed','offline','pending'])('primary validation stays available when shading is %s',async(mode)=>{
 job={id:'job',status:'failed',error:'Hauteur indisponible'};mount();const original=mocks.api.getMockImplementation()!;
 mocks.api.mockImplementation(async(url:string,opts:any={})=>{
  if(url.endsWith('/tree-shading/jobs')){
   if(mode==='offline')throw Error('Network unavailable');
   if(mode==='pending')return new Promise(()=>{});
  }
  return original(url,opts);
 });
 fireEvent.click(screen.getByText('Valider le calepinage'));
 await waitFor(()=>expect(validated).toBe(1));expect(saves).toBe(1);expect(mocks.navigate).toHaveBeenCalledOnce();expect(result).toBeNull();expect(excluded).toBe(false);
});
