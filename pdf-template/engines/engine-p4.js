...
(function () {
  ...
  function buildOverlayInputs(prefill) {
    ...
    for (let i = 1; i <= 12; i++) {
      const vProd = store[`p4_prod_m${i}`] ?? prodArr[i - 1] ?? "";
      const vConso = store[`p4_conso_m${i}`] ?? consoArr[i - 1] ?? "";
      const vAuto = store[`p4_auto_m${i}`] ?? autoArr[i - 1] ?? "";
      const vBatt = store[`p4_batt_m${i}`] ?? battArr[i - 1] ?? "";
  ...
  function openOverlay() {
    ...
    // **Correction de l'ordre d'appel** : Envoyer les données au graphique **après** l'overlay
    const rows = [];
    for (let i = 1; i <= 12; i++) {
      const prod = store[`p4_prod_m${i}`] || 0;
      const conso = store[`p4_conso_m${i}`] || 0;
      const auto = store[`p4_auto_m${i}`] || 0;
      const batt = store[`p4_batt_m${i}`] || 0;
      rows.push({ prod, conso, auto, batt });
    }
    drawChart(rows);  // Affichage immédiat du graphique avec les données
  }
...
`;
  ...
  function validateOverlay() {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      store[`p4_prod_m${i}`] = prod;
      store[`p4_conso_m${i}`] = conso;
      store[`p4_auto_m${i}`] = auto;
      store[`p4_batt_m${i}`] = batt;

      rows.push({ prod, conso, auto, batt });
    }

    store["p4_validated"] = true;
      ...
      <table style="width:100%;border-collapse:collapse;font-size:3.6mm;min-width:120mm;">
        ...
        <tbody>
          <tr style="border-bottom:0.2mm solid #f2f2f2">
            <td>Année</td>
            <td align="right">${Math.round(tot.prod)}</td>
            <td align="right">${Math.round(tot.conso)}</td>
            <td align="right">${Math.round(tot.auto)}</td>
            <td align="right">${Math.round(tot.batt)}</td>
          </tr>
        </tbody>
      </table>
    `;
  ...
  function hydrateFromBackend(payload) {
    ...
    // Si pas encore validé par l'utilisateur, on peut dessiner un graphe
    if (!store["p4_validated"]) {
      const rows = [];
      for (let i = 0; i < 12; i++) {
